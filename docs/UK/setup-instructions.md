...
- **Бюджети токенів LLM** — це не файл конфігурації. Вони зберігаються в базі даних бота, заповнюються значеннями за замовчуванням під час міграцій і редагуються на льоту через `/token_budget set`. (У старіших версіях використовувався `llm_token_budget.json`; якщо він досі лежить у `src/`, його більше ніхто не читає і його можна видалити.)
- **Перезавантаження когів.** `/reload_cogs` перезапускає ког, але **не** підхоплює зміни в `src/core/` чи `src/modules/` — для них потрібен повний перезапуск бота. Див. [про перезавантаження когів](/docs/UK/cog_hot_reloading.md).
