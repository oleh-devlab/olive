...
class ResilientTaskHandler:
    ...
    async def handle_error(self, error):
        ...
        if is_server_error or is_connection_error:
            ...
            if (delay == self.base_delay) or (
                delay == self.max_delay and self.base_delay * (2 ** (self.retries_5xx - 1)) < self.max_delay
            ):
                try:
                    error_channel = await self.bot.get_or_fetch_channel(channels["bot_news"])
                    if error_channel:
                        text = (
                            get_phrases(error_channel.guild.id)
        ...
        try:
            ...
            text = (
                get_phrases(error_channel.guild.id)
                .get("statistic_message_loop", {})
                .get("general_error_notification", "Task `{task_name}` issued an error: {error}")
