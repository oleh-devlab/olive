...
# The instruction this repository ships. Unlike every other prompt it describes
# the tools rather than a server, so it is the one that can live in git.
DEFAULT_AGENT_PROMPT_FILE = "prompts/schedule_agent.md"
...
def _get_schedule_instruction() -> str:
    """The agent's system instruction: `prompts/schedule_agent.md` unless the config names another."""
    instruction = get_instruction("schedule_agent", default_file=DEFAULT_AGENT_PROMPT_FILE)

    if not instruction:
        logger.error("Schedule agent instruction is empty (looked for %s)", DEFAULT_AGENT_PROMPT_FILE)
            ...
            while iteration < MAX_ITERATIONS:
                ...
                try:
                    response = await llm_client.get_interaction(
                        ...
                        model_priority=get_priority("schedule_agent"),
